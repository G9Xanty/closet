...
async function main() {
  ...
  for (const u of [SELLER, BUYER, THIRD]) {
    ...
    // Login via supabase directly to bypass Express auth rate limiter
  ...
  console.log("\n--- Case 6: Self-request blocked ---");
  let r = await api("POST", "/api/sale-requests", sellerToken, { product_id: productA });
  check("Seller cannot request own product", r.status === 400 && r.data.error?.includes("propia"), r.data.error);

  /* ── Case 1: Happy flow ── */
  console.log("\n--- Case 1: Happy flow ---");
  r = await api("POST", "/api/sale-requests", buyerToken, { product_id: productA });
  check("Buyer can request product", r.ok && r.data.request?.status === "requested", r.data.error);
  const reqA = r.data.request?.id;

  r = await api("GET", "/api/sale-requests/mine", buyerToken);
  check("Buyer sees request in asBuyer", r.data.asBuyer?.some(sr => sr.id === reqA), "not in list");

  r = await api("GET", "/api/sale-requests/mine", sellerToken);
  check("Seller sees request in asSeller", r.data.asSeller?.some(sr => sr.id === reqA), "not in list");

  r = await api("PATCH", `/api/sale-requests/${reqA}`, sellerToken, { status: "accepted" });
  check("Seller can accept request", r.ok && r.data.request?.status === "accepted", r.data.error);

  /* ── Case 2: Duplicates blocked ── */
  // Product A is already "reserved" (after accept), so availability check fires first
  // Both errors are valid: "Ya solicitaste" (duplicate) or "Prenda no disponible" (reserved)
  console.log("\n--- Case 2: Duplicates blocked ---");
  r = await api("POST", "/api/sale-requests", buyerToken, { product_id: productA });
  ...
  r = await api("POST", "/api/sale-requests", thirdToken, { product_id: productA });
  ...
  console.log("\n--- Case 4: Cancellation frees product ---");
  r = await api("POST", "/api/sale-requests", buyerToken, { product_id: productB });
  check("Buyer can request product B", r.ok, r.data.error);
  const reqB = r.data.request?.id;
  ...
  r = await api("PATCH", `/api/sale-requests/${reqB}`, buyerToken, { status: "cancelled" });
  check("Buyer can cancel own request", r.ok && r.data.request?.status === "cancelled", r.data.error);
  ...
  console.log("\n--- Case 5: Rejection frees product ---");
  r = await api("POST", "/api/sale-requests", buyerToken, { product_id: productC });
  check("Buyer can request product C", r.ok, r.data.error);
  const reqC = r.data.request?.id;
  ...
  r = await api("PATCH", `/api/sale-requests/${reqC}`, sellerToken, { status: "rejected" });
  check("Seller can reject request", r.ok && r.data.request?.status === "rejected", r.data.error);
